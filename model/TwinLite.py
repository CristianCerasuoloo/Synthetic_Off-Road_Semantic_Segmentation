...
class TwinLiteNet(nn.Module):
    ...
    def __init__(self, p=2, q=3 ):
        ...
        super().__init__()
        #self.freeze_b1_weights = freeze_b1_weights
...
class TwinLiteNetRGBD(nn.Module):
    '''
    This class defines the ESPNet network
    '''
    ...
    def __init__(self, p=2, q=3 ):
        ...
        super().__init__()
        #self.freeze_b1_weights = freeze_b1_weights
        self.encoder_RGB = ESPNet_Encoder(p, q)
        self.encoder_D = ESPNet_Encoder(p, q)
        self.up_1_1 = UPx2(64,16)
        self.up_2_1 = UPx2(16,8)
        self.classifier_1 = UPx2(8,2)

    def forward(self, rgb,d):

        x_rgb=self.encoder_RGB(rgb)
        x_d=self.encoder_D(d)
...
class AdaptiveFusionModule(nn.Module):
    def __init__(self, in_channels_rgb, in_channels_depth, out_channels):
        ...
        # Define a convolutional layer for the RGB stream
        self.rgb_conv = nn.Conv2d(in_channels_rgb, out_channels, kernel_size=3, padding=1)
        
        # Define a convolutional layer for the depth stream
        self.depth_conv = nn.Conv2d(in_channels_depth, out_channels, kernel_size=3, padding=1)
        
        # Define a learnable parameter (or weight) for adaptive fusion
        self.alpha = nn.Parameter(torch.tensor(0.5))

        # Optional: You might want to add a BatchNorm layer after the convolution
        self.bn = nn.BatchNorm2d(out_channels)
        
        # Optional: Add an activation function
        self.relu = nn.ReLU(inplace=True)

    def forward(self, rgb, depth):
        # Pass the RGB input through its convolutional layer
        rgb_feat = self.rgb_conv(rgb)
        
        # Pass the depth input through its convolutional layer
        depth_feat = self.depth_conv(depth)
        # Perform the adaptive fusion
        fused_feat = self.alpha * rgb_feat + (1 - self.alpha) * depth_feat
        
        # Optionally apply BatchNorm and activation function
        fused_feat = self.bn(fused_feat)
        fused_feat = self.relu(fused_feat)
        
        return fused_feat

class TwinLiteNet_RGBD_Adaptive(nn.Module):
    def __init__(self, p=2, q=3):
        super().__init__()
        # Define the RGB stream
        self.rgb_stream = ESPNet_Encoder(p,q)

        
        
        # Define the depth stream
        self.depth_stream = ESPNet_Encoder(p,q)

        
        # Define the adaptive fusion module
        self.adaptive_fusion = AdaptiveFusionModule(in_channels_rgb=32, in_channels_depth=32, out_channels=32)
    ...
    def forward(self, rgb, depth):
        # Pass the RGB input through the RGB stream
        rgb_feat = self.rgb_stream(rgb)
        
        # Pass the depth input through the depth stream
        depth_feat = self.depth_stream(depth)
        
        # Perform adaptive fusion
        fused_feat = self.adaptive_fusion(rgb_feat, depth_feat)
        
        # Perform upsampling
        up_1 = self.up_1_1(fused_feat)
        up_2 = self.up_2_1(up_1)
