...
if __name__ == "__main__":
    ...
    video_folder = "/user/ccerasuolo/workspace/v6"
    video_rgb_folder = video_folder + "/RGB"
    video_gt_folder = video_folder + "/GT"

    frame_folder = "/user/ccerasuolo/workspace/SynthOffRoadV6"
    ...
    for video_path in video_files:
        if "Rainy" in video_path:
            print("Skipping rainy ", video_path)
            continue


        if "WL" in video_path and "GT" in video_path:
            if not "inv" in video_path:
                # if the inv version of that video is not present, create it
                ext = video_path.split(".")[-1]
                inv_video_path = video_path.replace("." + ext, "_inv." + ext)
                print(inv_video_path)
                if not os.path.exists(inv_video_path):
                    invert_grayscale_video(video_path, inv_video_path)
                    video_path = inv_video_path
                else:
                    print("Skipping already inverted: ", video_path)
                    continue
