...
class VideoProcessingGUI(QMainWindow):
    ...
    def initUI(self):
        ...
        # Aggiunta di un pulsante per selezionare il Ground Truth video
        self.gt_video_button = QPushButton('Select GT Video')
        self.gt_video_button.clicked.connect(self.select_gt_video)
        controls_layout.addWidget(self.gt_video_button)

        # Variabile per memorizzare il percorso del video GT
        self.gt_video_path = ''
        self.cap_gt = None
    ...
    def select_gt_video(self):
        self.clear()
        self.gt_video_path, _ = QFileDialog.getOpenFileName(self, "Select GT Video", "", "Video Files (*.mp4 *.mkv *.avi)")
        if self.gt_video_path:
            self.gt_video_button.setText(f"GT Video: {self.gt_video_path.split('/')[-1]}")
    ...
    def start_processing(self):
        ...
        # Aggiunta del video GT
        if self.gt_video_path:
            self.cap_gt = cv2.VideoCapture(self.gt_video_path)
    ...
    def update_frame(self):
        ...
        ret_gt, frame_gt = self.cap_gt.read() if self.cap_gt else (False, None)

        if modality == 'RGB only' and ret_rgb:
            processed_frame = Run_rgb(self.model, frame_rgb, self.device)
            self.display_frames(frame_rgb, processed_frame, frame_gt)

        elif modality == 'DEPTH only' and ret_depth:
            processed_frame = Run_depth(self.model, frame_depth, self.device)
            self.display_frames(frame_depth, processed_frame, frame_gt)

        elif modality == 'Data Fusion' and ret_rgb and ret_depth:
            processed_frame = Run_datafusion(self.model, frame_depth, frame_rgb, self.device)
            self.display_frames((frame_rgb, frame_depth), processed_frame, frame_gt)
    ...
    def display_frames(self, original_frame, processed_frame, gt_frame=None):
        modality = self.modality_combo.currentText()
        ...
        # Converti il frame elaborato in RGB
        processed_rgb = cv2.cvtColor(processed_frame, cv2.COLOR_BGR2RGB)

        if gt_frame is not None:
            # Resize del GT frame per uniformità
            gt_frame_resized = cv2.resize(gt_frame, (processed_rgb.shape[1], processed_rgb.shape[0]))
            gt_gray = cv2.cvtColor(gt_frame_resized, cv2.COLOR_BGR2GRAY)

            # Creiamo una maschera per i pixel GT = 0
            gt_mask = (gt_gray == 0)  # True dove i pixel GT sono esattamente neri

            # Creiamo una maschera per i pixel verdi nella prediction
            green_mask = (processed_rgb[:, :, 0] == 0) & (processed_rgb[:, :, 1] == 255) & (processed_rgb[:, :, 2] == 0)

            # Maschera finale: GT nero E prediction verde
            combined_mask = gt_mask & green_mask


            overlay = rgb_frame.copy()

            # Select the indices of the image where the mask is 0 (black pixels) and the processed frame is green (0, 255, 0)
            overlay[combined_mask] = [255, 0, 0]

            
        else:
            overlay = processed_rgb

        h, w, ch = overlay.shape
        bytes_per_line = ch * w
        qt_image_processed = QImage(overlay.data, w, h, bytes_per_line, QImage.Format_RGB888)
